...
from utils.users.amount_of_users_getter import get_amount_of_users


async def welcome_user(message: types.Message) -> str:
    f = open(file="./data/message_texts/welcome.txt", mode="r+", encoding="utf-8")
...
async def get_gpt_request_limit_text(available_amount_of_requests: int, model: str) -> str:
    f = open(file="./data/message_texts/gpt_requests_limit_has_been_reached.txt", mode="r+", encoding="utf-8")
...
async def get_statistics_text() -> str:
    f = open(file="./data/message_texts/statistics.txt", mode="r+", encoding="utf-8")
