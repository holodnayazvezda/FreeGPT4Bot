from bot_data.config import LENGTH_OF_GPT3_HISTORY_FOR_USERS, LENGTH_OF_GPT4_HISTORY_FOR_USERS
...
async def add_request_to_history(database_name: str, table_name: str, user_id: int, request_text: str,
                                 role: str) -> None:
...
async def get_history_of_requests(database_name: str, table_name: str, user_id: int, model: str) -> list:
    conn = sqlite3.connect(database_name)
    c = conn.cursor()
    c.execute(f'CREATE TABLE IF NOT EXISTS {table_name} (id TEXT, requests TEXT)')
    ...
    if data:
        ...
        if len(data) > LENGTH_OF_GPT4_HISTORY_FOR_USERS:
            data = []
            await clear_history_of_requests("./bot_data/databases/history_of_requests_to_chatgpt.sqlite3",
                                            "users_history", user_id)
        elif model == 'gpt-3.5-turbo':
            if len(data) >= LENGTH_OF_GPT3_HISTORY_FOR_USERS:
                data = data[-LENGTH_OF_GPT3_HISTORY_FOR_USERS:]
        return data
    else:
        return []
