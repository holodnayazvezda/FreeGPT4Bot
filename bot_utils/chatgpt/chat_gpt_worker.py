import asyncio
import tiktoken
from threading import Thread
from openai import AsyncOpenAI

from bot_utils.async_process_runner import async_functions_process_starter
from bot_utils.chatgpt.apikeys_worker import start_or_stop_api_key, update_api_keys
from bot_utils.chatgpt.chat_gpt_users_worker import *
from bot_utils.chatgpt.requests_counter import *

counter_of_requests = 0


async def ask_chat_gpt_and_return_answer(model: str, prompt: str, user_id: int, recursion_len: int = 0) -> tuple:
    tokens_in_response = len(tiktoken.encoding_for_model("gpt-3.5-turbo").encode(prompt)) + 7
    global counter_of_requests
    from bot_utils.chatgpt.apikeys_worker import OPENAI_API_KEYS
    history_of_requests = await get_history_of_requests("./bot_data/databases/history_of_requests_to_chatgpt.sqlite3",
                                                        "users_history", user_id, model)
    history_of_requests.append({'role': 'user', 'content': prompt})
    if tokens_in_response > 4096:
        return None, 400
    table_name = 'quantity_of_requests_to_gpt3'
    api_key = OPENAI_API_KEYS[counter_of_requests % len(OPENAI_API_KEYS)]
    counter_of_requests += 1
    try:
        response = await AsyncOpenAI(api_key=api_key).chat.completions.create(
            model=model,
            messages=history_of_requests,
            temperature=0.1
        )
        response_content = response.choices[0].message.content
        await add_request_to_history("./bot_data/databases/history_of_requests_to_chatgpt.sqlite3", "users_history",
                                     user_id, prompt, 'user')
        await add_request_to_history("./bot_data/databases/history_of_requests_to_chatgpt.sqlite3", "users_history",
                                     user_id, response_content, 'assistant')
        Thread(target=async_functions_process_starter, args=(increase_the_number_of_requests_for_the_user, [
            "./bot_data/databases/quantity_of_requests.sqlite3", table_name, user_id])).start()
        return response_content.replace("\\n", "").strip(), 200
    except Exception as e:
        if 'gpt-4' in model:
            return await ask_chat_gpt_and_return_answer('gpt-3.5-turbo', prompt, user_id)
        if 'exceeded' in str(e) and 'quota' in str(e) and 'check' in str(e) and 'plan' in str(e):
            Thread(target=async_functions_process_starter, args=(start_or_stop_api_key, [1, api_key, 0])).start()
            await update_api_keys()
            if recursion_len >= 3:
                return None, 429
            else:
                await ask_chat_gpt_and_return_answer(model, prompt, user_id, recursion_len + 1)
        elif 'maximum' in str(e) and 'context' in str(e) and 'length' in str(e) and 'tokens' in str(e):
            Thread(target=async_functions_process_starter, args=(clear_history_of_requests, [
                "./bot_data/databases/history_of_requests_to_chatgpt.sqlite3", "users_history", user_id])).start()
            if recursion_len >= 2:
                return None, 429
            else:
                await ask_chat_gpt_and_return_answer(model, prompt, user_id, recursion_len + 1)
        else:
            return None, 500


if __name__ == '__main__':
    asyncio.run(ask_chat_gpt_and_return_answer("gpt-3.5-turbo", "привет", 112))
